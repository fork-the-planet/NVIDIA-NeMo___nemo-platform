...
# Regex to detect Python exception lines and extract the type name (group 1)
# and message (group 2) as separate captures. The type name may be module
# qualified (e.g. "subprocess.TimeoutExpired", "torch.cuda.OutOfMemoryError");
# _extract_exception strips the module prefix down to the bare class name.
# The suffix set deliberately goes beyond Error/Exception so non-"*Error"
# exceptions like TimeoutExpired / KeyboardInterrupt / SystemExit are captured.
_EXCEPTION_RE = re.compile(
    r"\b((?:[A-Za-z_]\w*\.)*[A-Z]\w*(?:Error|Exception|Expired|Interrupt|Exit)):\s*(.*)",
)
...
def _extract_exception(line: str) -> ParsedError | None:
    ...
    Examples:
        ...
        >>> _extract_exception("subprocess.TimeoutExpired: Command timed out")
        ParsedError(exception_type='TimeoutExpired', message='Command timed out')
    ...
    if match:
        # Strip any module prefix (e.g. "subprocess.TimeoutExpired" -> "TimeoutExpired")
        # so the bare class name matches the converter's type_name rules.
        exc_type = match.group(1).strip().rsplit(".", 1)[-1]
