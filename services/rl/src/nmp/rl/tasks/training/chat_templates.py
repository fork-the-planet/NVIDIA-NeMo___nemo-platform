...
def _get_tokenizer_chat_template(model_path: str, trust_remote_code: bool = False) -> str | None:
    """
    ...
    Args:
        ...
        trust_remote_code: Whether to allow executing the model repo's custom code
            when loading its tokenizer. Sourced from the model entity's
            ``trust_remote_code`` flag (operator/registry-controlled, threaded down
            from the API) — not free-form end-user input — because some models
            require it to load their tokenizer at all.
    """
    try:
...
def resolve_chat_template(
    ...
    Args:
        ...
        trust_remote_code: Forwarded to the tokenizer fallback. Sourced from the
            model entity's ``trust_remote_code`` flag; some models require it to
            load their tokenizer.
    ...
    template = _get_tokenizer_chat_template(model_path, trust_remote_code=trust_remote_code)
    if template:
...
def apply_chat_template_to_checkpoint(
    ...
    # Backfill pad_token from eos_token when missing. Many inference frameworks
    # require a pad_token, and full-weight checkpoints sometimes ship without one.
    if not config.get("pad_token") and config.get("eos_token"):
        config["pad_token"] = config["eos_token"]
        logger.info("Backfilled missing pad_token from eos_token")
